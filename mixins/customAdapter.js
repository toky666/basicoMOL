...
class MongoDbAdapter {
	 ...
	 */
	/*
	pipeline (query, callback) {
		return this.collection.aggregate(query).toArray((err, result) => {
			if (err) {
				// return this.Promise.reject(new MoleculerClientError(err, 404));
				callback (false, null);
			} else {
				callback (true, result);
				// return this.Promise.resolve(result);
				// console.log("consulta " + JSON.stringify(result));
				// return result;
			}
		});
	}
